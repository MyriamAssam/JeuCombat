...
public class ActionsActives {
	...
	public void activer(Action action) {
		...
		Ntro.newEvent(EvtActionJoueur.class).setPosition(position).setAction(action).trigger();
	}
	...
	public void desactiver(Action action) {
		...
		if (actions.isEmpty()) {
			...
			Ntro.newEvent(EvtActionJoueur.class).setPosition(position).setAction(Action.ARRETER).trigger();

		} else {

			for (Action actionRestante : actions) {

				Ntro.newEvent(EvtActionJoueur.class).setPosition(position).setAction(actionRestante).trigger();
			}
		}
	}
}
